...
export class AppService {
    ...
    async login(credentials: ILoginRequestDto): Promise<ITokensResponseDto> {
        const user = this.validateUserPassword(credentials, await this.findByEmail(credentials.email)).toJSON();
        const accessTokenPayload: TokenPayload = {email: user.email, id: user._id, roles: user.roles, sub: user.id};
        const accessToken = this.jwtService.sign(accessTokenPayload, this.accessTokenConfig);

        const refreshTokenPayload: TokenPayload = {email: user.email, id: user._id, roles: user.roles, sub: user.id};
